...
(()=>{'use strict';
 ...
 async function json(url,{ttl=300000,refresh=false,timeout=6000,validate=x=>x&&typeof x==='object'}={}){
  const key=String(url),cached=cache.get(key),time=Date.now();
  ...
  const task=(async()=>{let failure;
   for(let attempt=0;attempt<2;attempt++)try{
    ...
    cache.delete(key);cache.set(key,{value,at:Date.now()});bound(cache);blocked.delete(key);log.set(key,{status:'Current',lastSuccessAt:new Date().toISOString(),lastAttemptAt:new Date().toISOString()});bound(log);return value;
 ...
 api=requestAPI;
 window.RecoveryRequests={json,api:requestAPI,status:()=>[...log].map(([url,state])=>({url:url.replace(/\?.*$/,''),...state}))};
})();
