...
loadSeed=async function loadPublicSeed(){
  ...
  const results=await Promise.allSettled(['index.json','directory.json','macro.json'].map(name=>publicResearchFile(name,{fresh:name!=='directory.json'})));
...
researchBundle=async function publicResearchBundle(t,{refresh=false}={}){
  ...
  const promise=(async()=>{
    ...
    try{
      const enriched=await publicResearchFile(t+'.json',{fresh:true});
...
function evidenceChapters(b,a,tr){
  const f=a.fin.latest,py=a.fin.priorYear,cf=b.financials?.cashFlowPeriod,annual=b.financials?.annual?.[0],chapters=[];
  if(f){
  ...
  if(cf){
    ...
    chapters.push(evidenceCard(3,'Cash generation, with the period kept intact',[
      `${cf.scope}: ${cf.startDate} through ${cf.endDate} (approximately ${months} months). Operating cash flow was ${evidenceMoney(cf.operatingCashFlow)}. Capital expenditure on property, plant and equipment was ${evidenceMoney(cf.capitalExpenditure)}. Free cash flow, calculated as those two reported values subtracted, was ${evidenceMoney(cf.freeCashFlow)}.`,
  ...
  if(f)chapters.push(evidenceCard(4,'Balance-sheet room to maneuver',[
    `At ${f.endDate}, reported cash and cash equivalents were ${evidenceMoney(f.cash)}. Current assets were ${evidenceMoney(f.currentAssets)} against ${evidenceMoney(f.currentLiabilities)} of current liabilities${MODEL.finite(f.currentRatio)?`, a current ratio of ${f.currentRatio.toFixed(2)}×`:''}. Total assets were ${evidenceMoney(f.assets)} and shareholder equity was ${evidenceMoney(f.equity)}.`,
  ...
  if(annual)chapters.push(evidenceCard(5,'Step back to the annual cycle',[
    `For ${annual.period}, ended ${annual.endDate}, revenue was ${evidenceMoney(annual.revenue)}, operating income ${evidenceMoney(annual.operatingIncome)}, and net income ${evidenceMoney(annual.netIncome)}. These annual observations provide context around the latest quarter without annualizing a short seasonal period.`,
    `Annual operating cash flow was ${evidenceMoney(annual.operatingCashFlow)}${MODEL.finite(annual.freeCashFlow)?`, with calculated free cash flow of ${evidenceMoney(annual.freeCashFlow)}`:''}. Compare the annual record with recent quarters to distinguish seasonal demand, a temporary expense shift and a longer operating trend. Use the financials destination for the underlying rows.`
