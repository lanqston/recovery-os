...
def refresh(public=None):
    ...
    try:
        ...
        prior={'source':url,'retrievedAt':public.received_at(url),'symbols':symbols};public.write(path,prior);fetched=True
        checked=public.success_health('SEC symbol directory',url)
        status={'status':'Current' if checked['status']=='available' else 'Cached','lastSuccessAt':checked['lastSuccessAt'],'lastAttemptAt':now,'records':len(symbols)}
    ...
    if fetched or not legacy.exists():
