...
function audit(){
 ...
 for(const file of files){
  ...
  for(const h of d.health||[])if(/error|blocked|limit|retained|fail|cooldown|paused|HTTP_|unavailable/i.test(h.status+' '+h.detail))add(ticker,'PROVIDER_FAILURE',h.provider+': '+h.detail);
 }
