...
FEEDS = {
    ...
    'AMD': ('AMD Investor Relations', 'https://ir.amd.com/news-events/press-releases/rss'),
...
def refresh():
    reports = []
    for ticker, (publisher, url) in FEEDS.items():
        path = public.DEST/(ticker+'.json')
        if not path.exists(): continue
        bundle = json.loads(path.read_text()); provider = 'Official issuer news'
        try:
            text = public.fetch(url, ticker+'-official-news', 'text', ttl=1800)
            items = public.feed_items(text, publisher, 12)
            ...
            prior = next((h for h in bundle.get('health', []) if h['provider']==provider), {})
